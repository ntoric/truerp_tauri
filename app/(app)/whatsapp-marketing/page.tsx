...
export default function WhatsAppMarketingPage() {
  ...
  const fetchStats = async () => {
    try {
      const res = await apiFetch('/whatsapp-marketing/stats')
  ...
  const handleViewDetails = async (campaign: WhatsAppCampaign) => {
    try {
      const res = await apiFetch(`/whatsapp-marketing/${campaign.id}`)
      if (res.ok) {
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateCampaign} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-gray-500">For image/video messages</p>
                </div>
                <div className="space-y-2">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">{stats.total_delivered}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-purple-600">{stats.total_read}</div>
              </CardContent>
            </Card>
          </div>
        )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left text-gray-500">
                      ...
                      <th className="pb-3 font-medium">Sent/Delivered/Read</th>
                    ...
                    {paginatedItems.map((campaign) => (
                      <tr key={campaign.id} className="border-b last:border-0 hover:bg-gray-50">
                        ...
                        <td className="py-3 text-gray-600">
                          ...
                          {campaign.delivered_count > 0 && <span className="text-blue-600"> / {campaign.delivered_count}</span>}
                          {campaign.read_count > 0 && <span className="text-purple-600"> / {campaign.read_count}</span>}
                        </td>
                    ...
                    {campaigns.length === 0 && (
                      <tr>
                        <td colSpan={7} className="py-8 text-center text-gray-500">
                          No WhatsApp campaigns created yet
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="space-y-4">
              ...
              {selectedCampaign.media_url && (
                <div>
                  <Label>Media</Label>
                  <div className="mt-2">
                    <a href={selectedCampaign.media_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label>Sent/Delivered/Read</Label>
                  <p className="text-sm">
                    <span className="text-green-600">{selectedCampaign.sent_count}</span>
                    {selectedCampaign.delivered_count > 0 && <span className="text-blue-600"> / {selectedCampaign.delivered_count}</span>}
                    {selectedCampaign.read_count > 0 && <span className="text-purple-600"> / {selectedCampaign.read_count}</span>}
                  </p>
                </div>
              </div>

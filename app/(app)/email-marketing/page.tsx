...
export default function EmailMarketingPage() {
  ...
  const filteredParties = parties.filter(p => {
    if (newCampaign.target_audience === 'specific_customers') return p.party_type === 'customer' && p.email
    if (newCampaign.target_audience === 'specific_vendors') return p.party_type === 'vendor' && p.email
    return false
  })
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Manage your email campaigns</p>
          </div>
              ...
              <form onSubmit={handleCreateCampaign} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewCampaign({ ...newCampaign, target_audience: value, party_ids: [], email_addresses: [] })}
                  >
                    <SelectTrigger>
                ...
                {newCampaign.target_audience === 'custom_emails' && (
                  <div className="space-y-2">
                    <Label htmlFor="email-addresses">Email Addresses (comma separated) *</Label>
                    <Input
                      id="email-addresses"
                      value={newCampaign.email_addresses.join(', ')}
                      onChange={(e) => setNewCampaign({ ...newCampaign, email_addresses: e.target.value.split(',').map(e => e.trim()).filter(e => e) })}
